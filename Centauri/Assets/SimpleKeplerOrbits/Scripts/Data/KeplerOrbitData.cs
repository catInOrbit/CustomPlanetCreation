    ...
    public class KeplerOrbitData
    {
        ...
        public void CalculateNewOrbitData()
        {
            ...
            if (Eccentricity < 1)
            {
                ...
                Period = 2 * Mathf.PI * Mathf.Sqrt(Mathf.Pow((float)SemiMajorAxis, 3) / ((float)GravConst * ((float)AttractorMass + (float)thisObjectMass)));
        ...
        public void GetOrbitPointsNoAlloc(ref Vector3[] orbitPoints, int pointsCount, Vector3 origin, float maxDistance = 1000f)
        {
            ...
            if (Eccentricity < 1) //Orbit is elliptical
            {
        ...
        public void GetOrbitPointsNoAlloReentry(ref Vector3[] orbitPoints, int pointsCount, Vector3 origin, float maxDistance = 1000f)
        {
            if (pointsCount < 2)
            {
                orbitPoints = new Vector3[0];
            ...
            if (Eccentricity < 1) //Orbit is elliptical
            {
                ...
                if (ApoapsisDistance < maxDistance)
                {
                    for (var i = 0; i < pointsCount; i++)
                    {
                        if ((Vector3)GetFocalPositionAtEccentricAnomaly(i * KeplerOrbitUtils.PI_2 / (pointsCount - 1d)) + origin == GameObject.FindGameObjectWithTag("Player").transform.position)
                            orbitPoints[i] = (Vector3)GetFocalPositionAtEccentricAnomaly(i * KeplerOrbitUtils.PI_2 / (pointsCount - 1d)) + origin;
                    }
                }
                else
                {
                    ...
                    for (int i = 0; i < pointsCount; i++)
                    {
                        if ((Vector3)GetFocalPositionAtTrueAnomaly(-maxAngle + i * 2d * maxAngle / (pointsCount - 1)) + origin == GameObject.FindGameObjectWithTag("Player").transform.position)
                            orbitPoints[i] = (Vector3)GetFocalPositionAtTrueAnomaly(-maxAngle + i * 2d * maxAngle / (pointsCount - 1)) + origin;
                    }
                }
            }
